...
#define LOG_HEADER		"%18s | %8s\n"
#define LOG_FORMAT		"%18s | %08x | %s\n"
...
int LOG_WriteFormattedLine(char * I_str);
...
int LOG_Init(char * I_filename, int I_logLevel) {
	...
	if (M_logFile == NULL) {
		...
		if (M_logFile == NULL) {
			...
			strLen = snprintf(buff, sizeof(buff), LOG_HEADER, "Date", "ThreadID");
			if (strLen > 0) {
				LOG_WriteLine("--------------------------------------------------------------------------------------------\n");
...
int LOG_WriteFormattedLine(char * I_str) {
	...
	snprintf(buff, sizeof(buff), LOG_FORMAT, timeStr, getpid(), I_str);
...
int LOG_Write(char * I_format, ...) {
	...
	return LOG_WriteFormattedLine(buff);
}
